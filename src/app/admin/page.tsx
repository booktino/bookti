...
const STATUS_STYLES: Record<CalendarBookingStatus, string> = {
  kommende: "bg-[#0F6E56]/10 text-[#0F6E56]",
  ferdig: "bg-[#EFF8F4] text-[#7A9A8E]",
  kansellert: "bg-red-50 text-red-500",
};
                ...
                <div className="min-w-0 flex-1 rounded-xl border border-[#C8E6D8] bg-white p-4 shadow-sm sm:p-6">
                  <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
                    <div className="flex items-center gap-2">
                      <button
                        onClick={prevMonth}
                        className="flex h-9 w-9 items-center justify-center rounded-lg border border-[#C8E6D8] text-[#0F6E56] transition-colors hover:bg-[#EFF8F4]"
                        aria-label="Forrige måned"
                      >
                        ‹
                      </button>
                      <h2 className="min-w-[10rem] text-center text-base font-bold capitalize sm:text-lg">
                  ...
                  <div className="mb-1 grid grid-cols-7 gap-1">
                    {WEEKDAYS.map((day) => (
                      <div
                        key={day}
                        className="py-2 text-center text-[10px] font-bold tracking-wide text-[#7A9A8E] uppercase sm:text-xs"
                      >
                        {day}
                      </div>
                    ))}
                        ...
                        <button
                          ...
                          className={`relative flex aspect-square min-h-[3rem] flex-col items-start rounded-lg border p-1.5 text-left transition-all sm:min-h-[4.5rem] sm:p-2 ${
                            isToday
                              ...
                              : hasBookings
                                ? "border-[#5DCAA5]/40 bg-[#5DCAA5]/10 hover:border-[#5DCAA5]"
                                : "border-[#C8E6D8]/60 bg-[#EFF8F4]/50 hover:border-[#C8E6D8] hover:bg-white"
                          } ${isSelected && !isToday ? "ring-2 ring-[#0F6E56] ring-offset-1" : ""}`}
                        >
                          <span
                            className={`text-xs font-bold sm:text-sm ${
                              isToday ? "text-white" : "text-[#0D3B2E]"
                            }`}
                          >
                            {day}
                          </span>
                  ...
                  <>
                    ...
                    <aside className="fixed top-0 right-0 z-50 flex h-full w-full max-w-sm flex-col border-l border-[#C8E6D8] bg-white shadow-xl transition-transform duration-300 ease-out lg:relative lg:z-auto lg:h-auto lg:w-80 lg:shrink-0 lg:rounded-xl lg:border lg:shadow-sm">
                      <div className="flex items-center justify-between border-b border-[#C8E6D8] px-4 py-4 sm:px-5">
                        <div>
                          <h3 className="text-sm font-bold text-[#0D3B2E]">
                        ...
                        <button
                          onClick={() => setSelectedDay(null)}
                          className="flex h-8 w-8 items-center justify-center rounded-lg text-[#7A9A8E] hover:bg-[#EFF8F4]"
                          aria-label="Lukk"
                        >
                          ✕
                        </button>
                        ...
                        {selectedBookings.length === 0 ? (
                          ...
                          <div className="space-y-2">
                            {selectedBookings.map((b) => (
                              <div
                                key={b.id}
                                className={`rounded-xl border border-[#C8E6D8] bg-[#EFF8F4]/40 px-4 py-3 ${
                              ...
                              >
                                ...
                                <div className="mt-1 text-sm font-semibold text-[#0D3B2E]">
                                  {b.customerName}
                                </div>
                                <div className="text-xs text-[#7A9A8E]">
                                  {serviceName(b.serviceId)}
                                </div>
                                <div className="mt-1 text-xs text-[#4A6B5E]">
